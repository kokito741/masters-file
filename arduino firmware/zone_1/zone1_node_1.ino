 ...
 *  ---------------------------------------------------------------
 *  Zone 1 node firmware   |   ESP32-32U DevKit (38-pin)
 *
 *  Data path:  ESP32 --MQTT/UltraLight2.0--> IoT Agent --> Orion --> Mongo
 *
 *  PIN MAP
 *    GPIO4   DHT22            air temperature + humidity  (set HAS_DHT 0 to drop)
 *    GPIO34  Water level      reservoir probe, ANALOG output (was GPIO15)
 *    GPIO21  I2C SDA          DS3231 RTC
 *    GPIO22  I2C SCL          DS3231 RTC
 *    GPIO25  Valve relay      -> wire to IN1 on the shared 4-ch board
 *    GPIO26  YF-B2 flow       via 10k/3k3 divider, interrupt input
 *    GPIO33  Pump relay       IN4, SHARED open-drain line
 *    GPIO16  RS485 RXD        SN3002 soil sensor (Serial2)
 *    GPIO17  RS485 TXD        SN3002 soil sensor (Serial2)
 *
 *  WIRING CHANGE - the reservoir level probe moved from GPIO15 to GPIO34 and
 *              now uses its ANALOG output. GPIO15 is on ADC2, which the ESP32
 *              cannot read while WiFi is active; GPIO34 is ADC1 and works.
 *              This also frees GPIO15, the strapping pin breaking uploads.
 *
 *  DRY-RUN    - the level probe gates the PUMP, not the plant. Below
 *              LEVEL_MIN_START no dose begins; below LEVEL_MIN_ABORT a dose
 *              already running stops at once. Running the SEAFLO dry
 *              damages it.
 *
 *  IRRIGATION - pulse-and-soak: deliver a fixed dose, wait for the water to
 *              reach the probe, then re-measure. Watering until the sensor
 *              reads wet would overwater badly, because soil moisture lags
 *              by minutes. See section 1b for all thresholds.
 *
...
#define DEVICE_ID            "zone001"          // matches IoT Agent provisioning
...
// ---- reservoir water-level sensor (ANALOG) ---------------------------
// Probe sits in the water container and reports how far up it is submerged.
// Wire the module's analog output to GPIO34. GPIO15 CANNOT be used: it is
// ADC2, and ADC2 is unavailable whenever WiFi is active on the ESP32.
//
// CALIBRATE: watch the [TANK] raw value on serial with the probe out of the
// water, then with the container full, and paste both numbers in. The mapping
// works in either direction, so it does not matter which is larger.
#define LEVEL_RAW_EMPTY       0        // raw ADC, probe dry (measured)
#define LEVEL_RAW_FULL        2000     // raw ADC, probe submerged (measured)
#define LEVEL_SAMPLES         8        // averaged per read, kills ADC noise

// Zone 1 was specified without a DHT. Set to 0 to drop air temp/humidity
// entirely (they will simply be absent from the payload).
#define HAS_DHT               1
...
// Reservoir cutout. Running the pump dry damages it, so these gate the pump
// rather than the plant. Two levels give hysteresis: refuse to START below
// _START, and ABORT a dose already running below _ABORT.
#define LEVEL_MIN_START       25.0f     // % - need at least this to begin
#define LEVEL_MIN_ABORT       12.0f     // % - stop immediately below this
#define LEVEL_BLOCKS_MANUAL   true      // false = manual command can override
...
// =====================================================================
#define PIN_DHT        4
#define PIN_LEVEL      34        // MOVED from 15: analog needs ADC1, not ADC2
...
struct Readings {
  ...
  float waterLevel = 0.0f;    // reservoir fill, 0..100 %
} rd;
...
int           levelRaw        = 0;     // last raw ADC value, for calibration
...
bool shouldStartWatering() {
  ...
  if (rd.waterLevel < LEVEL_MIN_START)           return false;  // reservoir too low
...
void irrigationController() {
  ...
  switch (irrState) {
    ...
    case IRR_WATERING: {
      ...
      // Reservoir ran down mid-dose: stop before the pump runs dry.
      if (rd.waterLevel < LEVEL_MIN_ABORT) {
        dailyVolume += (rd.volumeTot - doseStartVolume);
        alertMsg = "lowtank";
        setValve(false, "reservoir below cutout");
        lockoutUntilMs = now + SAFETY_LOCKOUT_MS;
        setIrrState(IRR_LOCKOUT);
        break;
      }
...
// Returns 0..100 % reservoir fill. The two calibration points may be in
// either order, so this works for probes whose raw value rises or falls
// with depth.
void readWaterLevel() {
  long sum = 0;
  for (uint8_t i = 0; i < LEVEL_SAMPLES; i++) { sum += analogRead(PIN_LEVEL); delay(2); }
  levelRaw = (int)(sum / LEVEL_SAMPLES);

  float span = (float)(LEVEL_RAW_FULL - LEVEL_RAW_EMPTY);
  float pct  = (fabsf(span) > 1.0f)
             ? ((levelRaw - LEVEL_RAW_EMPTY) * 100.0f / span) : 0.0f;
...
void updateFlow() {
  ...
  // GPIO33 picks up 50 Hz mains hum, which the pulse counter happily reads as
...
void publishMeasurements() {
  ...
  appendPair(payload, "wl", rd.waterLevel,   1);   // reservoir fill    %
...
void mqttCallback(char* topic, byte* payload, unsigned int len) {
  ...
  if (cmdName == "valve") {
    if (value == "open" || value == "on" || value == "1") {
      if (LEVEL_BLOCKS_MANUAL && rd.waterLevel < LEVEL_MIN_START) {
        alertMsg = "lowtank";
        sendCommandAck(cmdName, "REFUSED reservoir low");
        Serial.printf("[IRR ] manual open refused, tank %.1f %%\n", rd.waterLevel);
        return;
      }
      alertMsg      = "ok";
...
void setup() {
  ...
  analogSetPinAttenuation(PIN_LEVEL, ADC_11db);   // full 0..3.3 V range
  ...
  readWaterLevel();
  Serial.printf("[TANK] raw %d -> %.1f %% full (GPIO%d, ADC1)\n",
                levelRaw, rd.waterLevel, PIN_LEVEL);
  Serial.println("       calibrate: note raw with probe dry, then submerged,");
  Serial.println("       then set LEVEL_RAW_EMPTY / LEVEL_RAW_FULL");
  ...
  Serial.printf("[IRR ] tank cutout: start>=%.1f%%  abort<%.1f%%\n",
                LEVEL_MIN_START, LEVEL_MIN_ABORT);
